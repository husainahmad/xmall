...
function cari_member(){
	...
	$data['user_acount']=$this->data_model->get_user_acount($this->session->userdata('sesUserName'));
	$data['member']=$this->data_model->get_daftar_member_by_namalengkap($data['nama']);
	$data['main_content']='entry/member';
	$this->load->view('layout/template',$data);
}
...
function add_member(){
	$data['user_acount']=$this->data_model->get_user_acount($this->session->userdata('sesUserName'));
	$thismonth  = date('my');
	$MemberId = $this->data_model->get_max_id_member();
	$NewMemberId = $MemberId['MemberId'] + 1;
	$satu="0000";
	$dua = $NewMemberId;
	$tiga = substr_replace($satu, $dua, (strlen($satu) - strlen($dua)));
	$data['IdCard'] = "PC-".$thismonth."-".$tiga;
...
function detail_member(){
	...
	$data['transaksi']=$this->data_model->get_transaksi_by_id($MemberId);
